...
dance_scatter <- ggplot(toptracks,
  ...
  #geom_point(summarise_all(toptracks, mean), shape = 15)
  geom_rug(size = 0.1) +      # Add 'fringes' to show data distribution.
  facet_wrap(~category) +     # Separate charts per ...
  labs(
    x = "Tempo",
    y = "Danceability"
  ) +
...
sober <-
  get_tidy_audio_analysis("7iHAue5izVmII1Z6Q1xy7B") %>%
  select(segments) %>%
  unnest(segments) %>%
...
***
The plot on the left shows the chromagram of "Sober - Stripped" by Raye. This was an energy-outlier of my Top Tracks of 2019. I don't think this chromatogram is really informative for my corpus research, so I will probably not include it in my final portfolio.
